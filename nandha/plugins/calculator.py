...
```
- /calculator:
calculator any expression though our bot.
```
...
@app.on_message(filters.command(["calc", "calculate", "calculator"]) & ~filters.forwarded)
...
async def calc_cb(self, query):
    ...
    if query.from_user.id != int(uid):
        return await query.answer("Who are you??", show_alert=True, cache_time=3)
    try:
