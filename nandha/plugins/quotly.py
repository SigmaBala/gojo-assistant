...
```
- /q: Reply to message text for make quote type sticker format.
plus use /getsticker if you wanna get it as photo.
```
*Example*:
`/q` reply to message
"""
...
async def pyrogram_to_quotly(messages):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post("https://bot.lyo.su/quote/generate.png", json=payload) as r:
            if r.status == 200:
                return await r.read()
            else:
...
@bot.on_message(filters.command(["quotly", "q"]) & ~filters.forwarded)
        ...
        if check_arg[0]:
            if check_arg[1] < 2 or check_arg[1] > 10:
                return await message.reply(
                    "<code>❌ Arg must less than and equal to 2-9</code>"
                )
            try:
                messages = [
                    i
